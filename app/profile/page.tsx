...
export default function ProfilePage() {
  ...
  return <main className={pageBg}><div className="mx-auto max-w-6xl space-y-6"><section className="rounded-[2rem] bg-slate-950 p-6 text-white"><div className="flex flex-col items-center gap-5 text-center"><div className="flex h-28 w-28 items-center justify-center rounded-[2rem] bg-gradient-to-br from-emerald-300 via-white to-amber-200 text-4xl font-black text-slate-950 shadow-2xl">{initials}</div><div><p className="text-sm font-bold text-emerald-300">Character Sheet</p><h1 className="mt-1 text-4xl font-black">{name}</h1><p className="mt-2 text-xl font-black text-emerald-200">{stats.title}</p><p className="mt-1 text-lg font-black" dir="rtl">{stats.titleArabic}</p><p className="mt-2 text-sm font-bold text-slate-300">{stats.titleHint}</p><p className="mt-2 text-sm font-bold text-slate-300">Challenge Day {currentDay} of {totalDays} • Overall {stats.overallRank} • {currentScore.toFixed(1)} / 100</p></div></div></section>
    ...
    <section className="grid gap-4 md:grid-cols-3"><div className={cardClass}><p className="text-sm font-bold text-slate-500">Challenge Score</p><p className="mt-1 text-3xl font-black">{currentScore.toFixed(1)} / 100</p></div><div className={cardClass}><p className="text-sm font-bold text-slate-500">Strongest Pillar</p><p className="mt-1 text-3xl font-black">{stats.isStarter ? "Not earned yet" : stats.strongest.name}</p></div><div className={cardClass}><p className="text-sm font-bold text-slate-500">Current Title</p><p className="mt-1 text-2xl font-black">{stats.title}</p></div></section>
