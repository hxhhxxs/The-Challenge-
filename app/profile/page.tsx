...
function scoreText(n: number) { return n > 0 && n < 1 ? n.toFixed(3) : n.toFixed(1); }
...
function PointCard({ label, arabic, value, description }: { label: string; arabic: string; value: number; description: string }) { return <div className="rounded-2xl bg-slate-50 p-4"><div className="flex items-start justify-between gap-3"><div><p className="text-xs font-black uppercase tracking-wide text-emerald-700">{arabic}</p><h3 className="mt-1 text-lg font-black text-slate-950">{label}</h3></div><p className="rounded-full bg-white px-3 py-1 text-sm font-black text-slate-950">{scoreText(value)} pts</p></div><p className="mt-2 text-xs font-bold leading-5 text-slate-500">{description}</p></div>; }
...
export default function ProfilePage() {
  ...
  const titleHint = isStarter ? "Reach 5 points to earn your first real title." : `Title earned with ${scoreText(currentScore)} total points.`;
