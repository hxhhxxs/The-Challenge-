...
function aggregateFromLogs(rows: Array<{ computed_points?: any }>) {
  const totals = rows.reduce((acc, row) => {
    const p = row.computed_points || {};
    ...
    return acc;
  }, { quwwah: 0, imaan: 0, sabr: 0, niyyah: 0, adab: 0 });
  return {
    quwwah: round3(totals.quwwah),
    imaan: round3(totals.imaan),
    sabr: round3(totals.sabr),
    niyyah: round3(totals.niyyah),
    adab: round3(totals.adab),
  };
}
...
export default function ProfilePage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      const savedPillars = ((record as any).pillar_scores || loadedDraft.pillar_scores || {}) as Record<string, number>;
      ...
      const finalPillars = logScore > 0 ? logPillars : {
        quwwah: safeNum(savedPillars.quwwah),
        imaan: safeNum(savedPillars.imaan),
        sabr: safeNum(savedPillars.sabr),
        niyyah: safeNum(savedPillars.niyyah),
        adab: safeNum(savedPillars.adab),
      };
      const finalScore = logScore > 0 ? logScore : savedScore;

      if (logScore > 0 && (Math.abs(logScore - savedScore) > 0.001 || totalFromPillars(savedPillars as any) === 0)) {
        await supabase.from("users").update({ current_score: finalScore, pillar_scores: finalPillars }).eq("id", record.id);
        setSourceNote("Rebuilt from saved daily logs");
      }
  ...
  const pillarTotals = {
    quwwah: safeNum(rawPillars.quwwah),
    imaan: safeNum(rawPillars.imaan),
    sabr: safeNum(rawPillars.sabr),
    niyyah: safeNum(rawPillars.niyyah),
    adab: safeNum(rawPillars.adab),
  };
  ...
  return <main className={pageBg}><div className="mx-auto max-w-6xl space-y-6"><section className="rounded-[2rem] bg-slate-950 p-6 text-white"><div className="flex flex-col items-center gap-5 text-center"><div className="flex h-28 w-28 items-center justify-center rounded-[2rem] bg-gradient-to-br from-emerald-300 via-white to-amber-200 text-4xl font-black text-slate-950 shadow-2xl">{initials}</div><div><p className="text-sm font-bold text-emerald-300">Character Sheet</p><h1 className="mt-1 text-4xl font-black">{name}</h1><p className="mt-2 text-xl font-black text-emerald-200">{displayTitle}</p><p className="mt-1 text-lg font-black" dir="rtl">{displayArabic}</p><p className="mt-2 text-sm font-bold text-slate-300">{titleHint}</p><p className="mt-2 text-sm font-bold text-slate-300">Challenge Day {currentDay} of {totalDays} • Overall {stats.overallRank} • {currentScore.toFixed(1)} / 100</p></div></div></section>
    <section className={cardClass}><div className="flex flex-col justify-between gap-4 md:flex-row md:items-end"><div><p className="text-sm font-black text-emerald-700">Point Categories</p><h2 className="text-3xl font-black">Where your points are going</h2><p className="mt-2 text-sm text-slate-600">These totals come from your saved daily logs. Small actions may show as 0.010 or 0.006 so you can see the real proportional credit.</p><p className="mt-1 text-xs font-black uppercase tracking-wide text-slate-400">{sourceNote}</p></div><Link href="/check-in" className="rounded-full bg-emerald-600 px-5 py-3 text-sm font-black text-white">Add points</Link></div><div className="mt-6 grid gap-4 md:grid-cols-2 lg:grid-cols-5"><PointCard label="Quwwah" arabic="القوة" value={pillarTotals.quwwah} description="Body, calories, water, steps, exercise, weight, and sleep." /><PointCard label="Imaan" arabic="الإيمان" value={pillarTotals.imaan} description="Salah, Qur'an memorization, review, dhikr, and worship." /><PointCard label="Sabr" arabic="الصبر" value={pillarTotals.sabr} description="Money discipline, screen discipline, and limits." /><PointCard label="Niyyah" arabic="النية" value={pillarTotals.niyyah} description="Personal goals, daily missions, and intention-based work." /><PointCard label="Adab" arabic="الأدب" value={pillarTotals.adab} description="Reflection, mood check-ins, character tasks, and photo proof." /></div></section>
