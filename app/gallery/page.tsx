...
export default function GalleryPage() {
  ...
  async function savePhoto() {
    setError(""); setMessage("");
    if (!preview) { setError("Choose a photo first."); return; }
    if (!title.trim()) { setError("Give the memory a short title."); return; }
    const date = todayKey();
    const alreadyBonusedToday = photos.filter((photo) => photo.date === date && (photo as any).adabBonusApplied).length;
    const bonus = alreadyBonusedToday < 5 ? 0.01 : 0;
    const photo: GalleryPhoto & { adabBonusApplied?: boolean } = { id: crypto.randomUUID(), date, title: title.trim(), caption: caption.trim(), pillar, dataUrl: preview, createdAt: new Date().toISOString(), adabBonusApplied: bonus > 0 };
    const nextPhotos = [photo, ...photos].slice(0, 200);
    const oldCheckin = draft.checkins?.[date] || {};
    const oldPoints = oldCheckin.computedPoints || oldCheckin.computed_points || {};
    const nextPoints = bonus > 0 ? { ...oldPoints, character: round3(safeNum(oldPoints.character) + bonus), total: round3(safeNum(oldPoints.total) + bonus) } : oldPoints;
    const nextPillars = { ...(draft.pillar_scores || {}), adab: round3(safeNum(draft.pillar_scores?.adab) + bonus) };
    const nextScore = round3(safeNum(draft.current_score) + bonus);
    const nextCheckins = { ...(draft.checkins || {}), [date]: { ...oldCheckin, date, computedPoints: nextPoints, updatedAt: new Date().toISOString() } };
    const nextDraft = { ...draft, galleryPhotos: nextPhotos, checkins: nextCheckins, pillar_scores: nextPillars, current_score: nextScore };
    const supabase = createSupabaseBrowserClient();
    const { error: updateError } = await supabase.from("users").update({ onboarding_draft: nextDraft, pillar_scores: nextPillars, current_score: nextScore }).eq("id", userId);
    if (updateError) { setError(updateError.message); return; }
    setDraft(nextDraft); setPhotos(nextPhotos); setTitle(""); setCaption(""); setPreview(""); setPillar("quwwah");
    setMessage(bonus > 0 ? "Photo saved ✓ +0.010 Adab bonus" : "Photo saved ✓ Daily photo bonus cap reached");
