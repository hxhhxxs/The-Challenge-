...
function pillarLabel(pillar: string) {
  return PILLARS.find((p) => p.id === pillar)?.label || "Memory";
}
...
export default function GalleryPage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      setUserId(record.id);
      setDraft(d);
      setPhotos(Array.isArray(d.galleryPhotos) ? d.galleryPhotos : []);
    }
    load();
  }, [router]);

  const filtered = useMemo(() => {
    const list = filter === "all" ? photos : photos.filter((photo) => photo.pillar === filter);
  ...
  async function chooseFile(file?: File) {
    ...
    try {
      ...
      setError(err?.message || "Image preview failed.");
    }
  }
  ...
  async function savePhoto() {
    ...
    const photo: GalleryPhoto = { id: crypto.randomUUID(), date: todayKey(), title: title.trim(), caption: caption.trim(), pillar, dataUrl: preview, createdAt: new Date().toISOString() };
    const nextPhotos = [photo, ...photos].slice(0, 200);
    const nextDraft = { ...draft, galleryPhotos: nextPhotos };
    const supabase = createSupabaseBrowserClient();
    const { error: updateError } = await supabase.from("users").update({ onboarding_draft: nextDraft }).eq("id", userId);
    if (updateError) { setError(updateError.message); return; }
    setDraft(nextDraft);
    setPhotos(nextPhotos);
    setTitle(""); setCaption(""); setPreview(""); setPillar("quwwah");
    setMessage("Photo saved to your private gallery ✓");
  ...
  return <main className={pageBg}><div className="mx-auto max-w-6xl space-y-6"><section className="rounded-[2rem] bg-slate-950 p-6 text-white"><p className="text-sm font-bold text-emerald-300">Private Gallery</p><h1 className="mt-1 text-4xl font-black">Your challenge memories.</h1><p className="mt-2 max-w-2xl text-slate-300">Save photos from meals, workouts, Qur’an sessions, family moments, and mission proof. Private by default.</p></section>
    ...
    <section className="rounded-[2rem] bg-amber-100 p-5 text-amber-950"><p className="text-sm font-black uppercase tracking-wide">Next upgrade</p><p className="mt-1 text-sm font-semibold">The next step is Supabase Storage upload plus a small Adab bonus for up to 5 photos per day.</p><Link href="/check-in" className="mt-4 inline-block rounded-full bg-amber-900 px-5 py-3 text-sm font-black text-white">Go log today</Link></section>
  </div><BottomNav /></main>;
}
